...
public function submitRequestindignecy(Request $request)
{
    ...
    $validatedData = $request->validate([
     ...
     // Check if there are any existing requests with the same reg_num and a status not equal to "Claimed", "Declined", or "Cancelled"
     $existingRequest = IndigencyRequest::where('reg_num', $user->reg_number)
     ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
     ->where('status', '!=', 'Declined') // Ensure status is not Declined
     ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
     ->where('purpose', $validatedData['purpose']) // Check if the purpose is the same
     ->first();

 if ($existingRequest) {
     return response()->json(['message' => 'You cannot submit a new request while there is an existing Queue request.'], 400);
 }
...
public function submitBusinessPermit(Request $request)
{
    ...
    $existingRequest = BusinessPermit::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
    ...
    /*$existingPermit = BusinessCessation::where('bname', $validator['bname'])
    ->first();

    if (!$existingPermit) {
    return response()->json(['message' => 'A business this name already exists.'], 400);
    }
   
    $existingCessationAddress = BusinessCessation::where('baddress', $validator['baddress'])->first();
    if (!$existingCessationAddress) {
        return response()->json(['message' => 'A business cessation request with this address already exists.'], 400);
    }*/
...
public function submitCessation(Request $request)
{
    ...
    $existingRequest = BusinessCessation::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
...
public function submitRequestcertificate(Request $request)
{
    ...
    $existingRequest = CertificateRequest::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
...
public function submitRequestsoloparent(Request $request)
{
    ...
    $existingRequest = SoloparentRequest::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
...
public function submitRequestftj(Request $request)
{
    ...
    $existingRequest = FTJRequest::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
...
public function submitRequestgoodmoral(Request $request)
{
    // Retrieve the user ID from the session
    $userId = $request->session()->get('userId');

    // Check if the user ID is found in the session
    if (!$userId) {
        return response()->json(['error' => 'User ID not found in session'], 400);
    }
    ...
    Log::info('User age:', ['age' => $user->age]);
    if ($user->age <= 18) {
        return response()->json(['message' => 'The Selected Person is Under 18 Below'], 400);
    }
    ...
    $validatedData = $request->validate([
        ...
        'copy' => 'required|integer|min:1|max:5', // Validate that copy is an integer between 1 and 5
        'purpose' => ['required', function ($attribute, $value, $fail) {
            if ($value === 'null') {
                $fail('Invalid purpose.');
            }
        }],
        'requirements' => 'required|mimes:pdf|max:5000' // Ensure file is a PDF and does not exceed 5MB
    ], [
        'voters.required' => 'Voters field is required.',
        'voters.in' => 'Voters field must be valid.',
        'name.required' => 'Name field is required.',
        'copy.required' => 'Copy field is required.',
        'copy.integer' => 'Copy field must be an integer.',
        'copy.min' => 'Copy field must be at least 1.',
        'copy.max' => 'Copy field must not exceed 5.',
        'purpose.required' => 'Purpose field is required.',
        'requirements.required' => 'Requirements field is required.',
        'requirements.mimes' => 'Requirements must be a PDF file.',
        'requirements.max' => 'Requirements file size must not exceed 5MB.'
    ]);
    $existingRequest = GoodMoral::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
    ...
    // Return a success response
    return response()->json(['success' => 'Good Moral request submitted successfully']);
}

public function submitRequestlawincome(Request $request)
{
    // Retrieve the user ID from the session
    $userId = $request->session()->get('userId');
    ...
    $existingRequest = Lawincome::where('reg_num', $user->reg_number)
    ->where('status', '!=', 'Claimed') // Ensure status is not Claimed
    ->where('status', '!=', 'Declined') // Ensure status is not Declined
    ->where('status', '!=', 'Cancelled') // Ensure status is not Cancelled
    ->first();
    ...
    // Return a success response
    return response()->json(['success' => 'Law Income request submitted successfully']);
}
public function getData(Request $request)
{
    ...
    $data['indigency_requests'] = IndigencyRequest::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['certificate'] = CertificateRequest::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['business_permit'] = BusinessPermit::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['business_cessation'] = BusinessCessation::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['first_time_job_seeker'] = FTJRequest::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['solo_parent'] = SoloparentRequest::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['goodmoral'] = GoodMoral::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();

$data['Lawincome'] = Lawincome::where('reg_num', $user->reg_number)
    ->where('status', '<>', 'Cancelled') // Exclude Cancelled status
    ->get();
...
public function cancelRequest(Request $request)
    {
        ...
        }else if($type == "Certificate of Good moral For Employment"){
            $request = Goodmoral::find($id);
        }else if($type == "Certificate of Law Income"){
            $request = Lawincome::find($id);
        }
