            ...
            endpoints.forEach((url, index) => {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#permit_unclaimed').text(response.Readytoclaimcount);
                        $('#permit_claimed').text(response.claimcount);
                    },
                    error: function(xhr, status, error) {
                        console.error(error);
                    }
                ...
                success: function(response) {
                    ...
                    if (response.length === 0) {
                        ...
                        $.each(response, function(index, item) {
                            var id = "BP_" + formatDatecontroll(item.created_at) + "_" + item.id;
                            var name = item.name;
...
$(document).ready(function() {
    ...
    $('.printall').click(function() {
        ...
        //alert(officer);
        $.ajax({
            url: '/generate-all-approved-pdfs-permit', // The route you defined in web.php
            type: 'POST',
            data: {
...
$(document).ready(function () {
    $('#confirmApprove').click(function () {
        var requestType = 'Business Permit'; // Change this dynamically if needed
...
$('.upload-bpermit').click(function (e) {
    ...
    toggleSpinner(button, true);
    e.preventDefault();
    var voters = $('#voters-request').val();
    var name = $('#names_display_bpermit-request').val();
    var address = $('#address_request').val();
    var age = $('#age_request').val();
    var cnum = $('#cnum_request').val();
    var email = $('#email_display').val();
    var copy = $('#copy_bpermit-request').val();
    var bname = $('#bname-request').val();
    var baddress = $('#baddress-request').val();
    var fileInput = $('#requirements_bpermit-request').val();
    ...
    $.ajax({
        url: '{{ route("Admin.submit.permit.request") }}',
        type: 'POST',
        data: formData,
        headers: {
