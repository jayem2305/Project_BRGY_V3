   ...
   <script src="../lib/wow/wow.min.js"></script>
    ...
    $(document).ready(function() {
        ...
        function updatePendingCount() {
            const endpoints = [
                "{{ route('count.pending.soloparent') }}"
            ...
            endpoints.forEach((url, index) => {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#soloparent_unclaimed').text(response.Readytoclaimcount);
                        $('#soloparent_claimed').text(response.claimcount);
                    },
                    error: function(xhr, status, error) {
                        console.error(error);
                    }
        ...
        var type = 'soloparent'; // Example type
                ...
                success: function(response) {
                    ...
                    if (response.length === 0) {
                        ...
                        $.each(response, function(index, item) {
                            var id = "SP_" + formatDatecontroll(item.created_at) + "_" + item.id;
                            var name = item.name;
...
$(document).ready(function() {
    ...
    $('.printall').click(function() {
        ...
        //alert(officer);
        $.ajax({
            url: '/generate-all-approved-pdfs-soloparent', // The route you defined in web.php
            type: 'POST',
            data: {
...
function toggleSpinner(button, show) {
    ...
    $('#confirmApprove').click(function () {
        var requestType = 'Solo Parents'; // Change this dynamically if needed
...
$(document).ready(function () {
    // Temporary container for children names
    let childrenNames = [];
    ...
    function displayChildren() {
        ...
        childrenNames.forEach((name, index) => {
            $('#childrenNames').append(
                `<li>${name} <button class="removeChildButton btn btn-danger btn-sm" data-index="${index}">Remove</button></li>`
            );
        });
    }
    $('#addChildButton').click(function () {
        ...
        if (childName) {
            childrenNames.push(childName); // Add name to array
            displayChildren(); // Refresh display
            $('#childNameInput').val(''); // Clear input field
        }
    });
     // Remove a child from the list
     $(document).on('click', '.removeChildButton', function () {
        const index = $(this).data('index');
        childrenNames.splice(index, 1); // Remove from array
        displayChildren(); // Refresh display
    });
    ...
    toggleSpinner(button, true);
    e.preventDefault();
    var voters = $('#voters_solo_request').val();
    var name = $('#names_display_request').val();
    var address = $('#address_request').val();
    var age = $('#age_request').val();
    var cnum = $('#cnum_request').val();
    var email = $('#email_display').val();
    var copy = $('#copy_solo_request').val();
    var type = $('#soloparent_request').val();
    var fileInput = $('#requirements_solo_request').val();
    ...
    formData.append('selectedChildren', JSON.stringify(childrenNames));

    console.log(childrenNames);
    var csrfToken = $('meta[name="csrf-token"]').attr('content');
    $.ajax({
        url: '{{ route("Admin.submit.soloparent.request") }}',
        type: 'POST',
        data: formData,
        headers: {
...
const mobileInput = document.getElementById('cnum_request');
