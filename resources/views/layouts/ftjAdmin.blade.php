            ...
            endpoints.forEach((url, index) => {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#ftj_unclaimed').text(response.Readytoclaimcount);
                        $('#ftj_claimed').text(response.claimcount);
                    },
                    error: function(xhr, status, error) {
                        console.error(error);
                    }
                ...
                success: function(response) {
                    ...
                    if (response.length === 0) {
                        ...
                        $.each(response, function(index, item) {
                            var id = "FTJ_"+formatDatecontroll(item.created_at) +"_"+item.id;
                            var name = item.name;
                            var date = formatDate(item.created_at);
                            var address = item.residents.address;
                            var type = "First-Time Job Seeker";
                            var purpose = item.type;
                            var phone_number = item.residents.cnum;
...
$(document).ready(function() {
    ...
    $('.printall').click(function() {
        ...
        //alert(officer);
        $.ajax({
            url: '/generate-all-approved-pdfs-FTJ', // The route you defined in web.php
            type: 'POST',
            data: {
...
$(document).ready(function () {
    $('#confirmApprove').click(function () {
        var requestType = 'First Time Job Seeker'; // Change this dynamically if needed
    ...
    toggleSpinner(button, true);
    e.preventDefault();
    var voters = $('#voters_ftj_request').val();
    ...
    var copy = $('#copy_ftj_request').val();
    var type = $('#ftjtypes_request').val();
    var numlive = $('#numberofliving_request').val();
    var dayslive = $('#typeofdays_request').val();
    var number_day = numlive + " " + dayslive;
    var pname = $('#pname_request').val();
    var page = $('#page_request').val();
    var paddress = $('#paddress_request').val();
    ...
    $.ajax({
        url: '{{ route("Admin.submit.ftj.request") }}',
        type: 'POST',
        data: formData,
        headers: {
